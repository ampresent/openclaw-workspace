...
- 实现 hosts.toml 配置文件解析（~/.config/UtopOS/hosts.toml）
...
- 添加 generation 描述读取：从 UtopOS-description 文件读取
- 添加配置读取路径校验：防止路径穿越
- 所有错误消息改为中文
- 添加单元测试：风险评估 + 包解析
...
- UtopOS-agent Rust 项目：axum HTTP server + 6 个端点全部实现
...
## 2026-04-12 (续) — 初始设计
- 从 evolution-os (Rocky/RPM/push) 转向 UtopOS (NixOS/GitOps/pull)
