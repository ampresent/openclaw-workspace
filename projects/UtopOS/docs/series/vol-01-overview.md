...
UtopOS 不运行守护进程、不维护常驻服务。它是一组 bash 脚本 + 一份 skill 文档。AI Agent 读 skill，调脚本，执行命令。
...
Nix、RPM、Conda 是三种完全不同的包管理系统。UtopOS 用统一的脚本接口（`evo-*`）屏蔽差异，agent 只需要知道 `evo-build pkg --patch f`，不需要关心底层是 `nix-build` 还是 `rpmbuild`。
...
Ansible 管配置文件，UtopOS 管包本身。互补，不竞争。
...
Docker 把问题推到镜像层。UtopOS 在系统层解决。

```
Docker 思路：应用问题 → 改 Dockerfile → rebuild image → redeploy
UtopOS 思路：系统问题 → 改源码 → 重新打包 → 包管理器安装
```

适用场景不同：Docker 适合应用部署，UtopOS 适合系统软件。

### vs NixOS Configuration

NixOS 本身通过 `configuration.nix` 管理一切。UtopOS 在此基础上提供 AI 辅助的源码级修复能力。
...
```
```bash
# UtopOS：当 configuration.nix 覆盖不了时
# 源码中的默认值有问题 → 修改源码 → overlay → nixos-rebuild
```

UtopOS 是 NixOS 的补丁层，不是替代品。
