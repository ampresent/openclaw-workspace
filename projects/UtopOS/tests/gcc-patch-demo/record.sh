#!/usr/bin/env bash
#
# record.sh — 使用 asciinema 录制 UtopOS GCC 修复演示
#
# 用法：
...
echo ""
echo "═══════════════════════════════════════════════════"
echo "  UtopOS GCC 修复演示 — asciinema 录制"
echo "═══════════════════════════════════════════════════"
echo ""
...
if [ "${1:-}" = "--upload" ]; then
    ...
    asciinema rec --title "UtopOS: GCC 编译修复工作流 (implicit-function-declaration)" \
                  --command "bash ${SCRIPT_DIR}/demo-auto.sh" \
                  --idle-time-limit 5
else
    echo "录制到本地文件..."
    asciinema rec "$RECORD_FILE" \
                  --title "UtopOS: GCC 编译修复工作流 (implicit-function-declaration)" \
                  --command "bash ${SCRIPT_DIR}/demo-auto.sh" \
                  --idle-time-limit 5
fi
