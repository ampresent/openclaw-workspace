...
let
  cfg = config.services.UtopOS.nextcloud;
in {
  options.services.UtopOS.nextcloud = {
    enable = mkEnableOption "UtopOS managed Nextcloud";
    ...
    backupEnable = mkOption {
      type = types.bool;
      default = false;
      description = "Enable automated backups via UtopOS";
    };
  ...
  config = mkIf cfg.enable {
    ...
    # Backup service (UtopOS managed)
    systemd.services."UtopOS-nextcloud-backup" = mkIf cfg.backupEnable {
      description = "UtopOS Nextcloud backup";
      script = ''
        set -eu
    ...
    systemd.timers."UtopOS-nextcloud-backup" = mkIf cfg.backupEnable {
    ...
    # UtopOS metadata
    environment.etc."UtopOS/services/nextcloud.json".text = builtins.toJSON {
      name = "nextcloud";
      type = "web-app";
      hostname = cfg.hostname;
      endpoints = {
