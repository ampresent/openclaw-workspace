# Jellyfin NixOS Module for UtopOS
#
# Managed Jellyfin media server with hardware transcoding, Nginx reverse proxy,
# and optional integration with other UtopOS services.
#
# Usage in configuration.nix:
#   imports = [ ./modules/jellyfin.nix ];
#   services.UtopOS.jellyfin.enable = true;
...
let
  cfg = config.services.UtopOS.jellyfin;
in {
  options.services.UtopOS.jellyfin = {
    enable = mkEnableOption "UtopOS managed Jellyfin media server";
  ...
  config = mkIf cfg.enable {
    ...
    # UtopOS metadata
    environment.etc."UtopOS/services/jellyfin.json".text = builtins.toJSON {
      name = "jellyfin";
      type = "media-server";
