...
```nix
{
  inputs.UtopOS.url = "github:your-org/UtopOS";

  outputs = { self, nixpkgs, UtopOS, ... }: {
    nixosConfigurations.myhost = nixpkgs.lib.nixosSystem {
      system = "x86_64-linux";
      modules = [
        UtopOS.nixosModules.default
        {
          services.UtopOS-agent = {
            enable = true;
            port = 7890;
            # tokenFile = "/run/secrets/UtopOS-token";  # optional
          };
        }
      ];
    };
...
|--------|------|---------|-------------|
| `enable` | bool | `false` | Enable the UtopOS-agent service |
...
- The agent runs as a dedicated `UtopOS` user with minimal permissions

## Service Management

```bash
# Status
systemctl status UtopOS-agent

# Logs
journalctl -u UtopOS-agent -f

# Restart
systemctl restart UtopOS-agent
