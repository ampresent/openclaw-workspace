#!/bin/bash
# agent-experiment.sh — Run the UtopOS self-healing experiment
# This script:
# 1. Starts OpenClaw gateway
# 2. Sends a prompt that triggers the UtopOS skill
...
echo "=== UtopOS Container Experiment ===" | tee "$RESULTS"
...
# Phase 3: Trigger the UtopOS skill via session
echo "--- Phase 3: Triggering agent self-healing ---" | tee -a "$RESULTS"
echo "Sending prompt: 'Read skills/UtopOS/SKILL.md. Execute the full system scan and fix ALL issues you find.'" | tee -a "$RESULTS"

# The experiment prompt — this should trigger the UtopOS skill
cat > /tmp/experiment-prompt.txt << 'PROMPT'
你是一个运行在容器中的 OpenClaw agent。

请读取 skills/UtopOS/SKILL.md 并严格按照其中的协议执行：
