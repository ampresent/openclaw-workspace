...
cd openclaw-workspace/projects/UtopOS
...
UtopOS has a built-in dev mode that simulates NixOS commands without a real system:
...
curl -X POST http://localhost:3030/api/dev/mode \
  ...
  -d '{"enabled": true, "mock_system": true, "mock_data_dir": "/tmp/UtopOS-dev"}'
```
...
- **Isolated state** — all mock data in `/tmp/UtopOS-dev/`
...
1. Use `services.UtopOS.<name>` namespace
2. Provide sensible defaults
3. Include `enable` option
4. Emit UtopOS metadata JSON to `/etc/UtopOS/services/`
...
claude mcp add UtopOS-dev -- node dist/index.js --hosts-config ./test-hosts.toml
```
