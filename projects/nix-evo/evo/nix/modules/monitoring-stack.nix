# Monitoring Stack NixOS Module for nix-evo
#
...
#   services.nix-evo.monitoring.enable = true;
...
let
  cfg = config.services.nix-evo.monitoring;
in {
  options.services.nix-evo.monitoring = {
    enable = mkEnableOption "nix-evo managed monitoring stack";
  ...
  config = mkIf cfg.enable {
    ...
    services.prometheus = mkIf cfg.enablePrometheus {
      ...
      scrapeConfigs = [
        ...
        {
          job_name = "nix-evo";
          static_configs = [{ targets = [ "localhost:$(cat /etc/nix-evo/port 2>/dev/null || echo 3030)" ]; }];
          ...
          groups = [{
            ...
            rules = [
              ...
              }
              {
                ...
                expr = "changes(nix_evo_generation[5m]) > 0";
                for = "0m";
    ...
    # nix-evo metadata
    # ============================================
    environment.etc."nix-evo/services/monitoring.json".text = builtins.toJSON {
