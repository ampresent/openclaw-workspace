...
pub async fn metrics_endpoint(
    ...
    // System metrics
    metrics.push_str("# HELP nix_evo_up Whether nix-evo agent is running\n");
    metrics.push_str("# TYPE nix_evo_up gauge\n");
    metrics.push_str("nix_evo_up 1\n\n");

    // CPU usage
    if let Ok(cpu) = get_cpu_usage().await {
        metrics.push_str("# HELP nix_evo_cpu_usage_percent CPU usage percentage\n");
        metrics.push_str("# TYPE nix_evo_cpu_usage_percent gauge\n");
        metrics.push_str(&format!("nix_evo_cpu_usage_percent {cpu:.1}\n\n"));
    }

    // Memory
    if let Ok((total, used, avail)) = get_memory_info().await {
        metrics.push_str("# HELP nix_evo_memory_total_bytes Total memory in bytes\n");
        metrics.push_str("# TYPE nix_evo_memory_total_bytes gauge\n");
        metrics.push_str(&format!("nix_evo_memory_total_bytes {total}\n\n"));

        metrics.push_str("# HELP nix_evo_memory_used_bytes Used memory in bytes\n");
        metrics.push_str("# TYPE nix_evo_memory_used_bytes gauge\n");
        metrics.push_str(&format!("nix_evo_memory_used_bytes {used}\n\n"));

        let pct = if total > 0 { (used as f64 / total as f64) * 100.0 } else { 0.0 };
        metrics.push_str("# HELP nix_evo_memory_usage_percent Memory usage percentage\n");
        metrics.push_str("# TYPE nix_evo_memory_usage_percent gauge\n");
        metrics.push_str(&format!("nix_evo_memory_usage_percent {pct:.1}\n\n"));
    }

    // Disk usage for common mount points
    if let Ok(disks) = get_disk_usage().await {
        metrics.push_str("# HELP nix_evo_disk_usage_percent Disk usage percentage\n");
        metrics.push_str("# TYPE nix_evo_disk_usage_percent gauge\n");
        for (mount, pct) in &disks {
            ...
            metrics.push_str(&format!("nix_evo_disk_usage_percent{{mount=\"{mount}\"}} {pct:.1}\n"));
        }
        metrics.push('\n');
    }
    ...
    if let Ok(gen) = get_current_generation().await {
        metrics.push_str("# HELP nix_evo_generation Current NixOS generation number\n");
        metrics.push_str("# TYPE nix_evo_generation gauge\n");
        metrics.push_str(&format!("nix_evo_generation {gen}\n\n"));
    }

    // Failed services count
    if let Ok(count) = get_failed_services_count().await {
        metrics.push_str("# HELP nix_evo_failed_services Number of failed systemd units\n");
        metrics.push_str("# TYPE nix_evo_failed_services gauge\n");
        metrics.push_str(&format!("nix_evo_failed_services {count}\n\n"));
    }
...
pub async fn get_observability_config(
    ...
    let config_path = "/etc/nix-evo/observability.json";
...
fn load_alert_state() -> Option<AlertState> {
    let path = "/var/lib/nix-evo/alert-state.json";
