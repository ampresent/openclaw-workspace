...
┌──────────────────▼──────────────────────────┐
│  决策层 (Skill)                              │
│  skills/nix-evo/SKILL.md                     │
│  - 工作流决策树                               │
│  - 用户交互协议                               │
│  - 风险评估规则                               │
│  - 信任白名单                                 │
│                                              │
│  skills/nix-evo-{nix,rpm,conda}/SKILL.md     │
│  - 后端专项知识                               │
│  - 排障指南                                   │
└──────────────────┬──────────────────────────┘
                   │
...
```
用户 → MCP Server (TypeScript) → nix-evo-agent (Rust, 39 endpoints) → NixOS
```
...
`skills/nix-evo/SKILL.md` 是 agent 的"操作手册"，包含：
...
| Skill | 深度内容 |
|-------|---------|
| nix-evo-nix | Nix 语言速查、overlay 机制、module 开发 |
| nix-evo-rpm | Spec 文件结构、rpmbuild 用法、发行版差异 |
| nix-evo-conda | Feedstock、meta.yaml、conda build |
