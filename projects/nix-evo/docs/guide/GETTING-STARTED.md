...
nix-evo 给你第三条路：**告诉 AI 你的问题，它帮你定位根因、修补源码、重新打包、安全安装。**
...
- nix-evo skill 已安装（见下方）

### 安装 Skill

把 `skills/nix-evo/` 目录放到你的 Claude Code workspace 的 skills 下：

```
~/.openclaw/workspace/skills/
├── nix-evo/SKILL.md          # 通用决策层
├── nix-evo-nix/SKILL.md      # Nix 后端（可选）
├── nix-evo-rpm/SKILL.md      # RPM 后端（可选）
└── nix-evo-conda/SKILL.md    # Conda 后端（可选）
```
...
Claude: 读 nix-evo-nix skill，结合实际案例讲解。
...
**Q: 这个工具适合生产环境吗？**
A: nix-evo 的设计就是面向生产环境的。所有变更先验证再安装，支持回滚，补丁可追溯。

**Q: 和直接 vim 改配置有什么区别？**
A: vim 改的是运行时文件，下次包更新会覆盖。nix-evo 改的是源码，通过包管理器安装，变更持久化、可追溯、可回滚。
