...
nix-evo 是一个 OpenClaw Skill，指导 AI Agent 遵循 source-first 的修复工作流：不绕过问题，不直接修改操作系统，而是解压源码、分析根因、修补源码、重新打包、通过包管理器安装。

### 核心场景

用户服务器出了问题，AI Agent 通过 nix-evo skill 自动检测包管理器、下载源码、诊断问题、生成补丁、重新打包、安全安装。
...
用户 ("nginx 502 了")
    │
    ▼
AI Agent (OpenClaw, 遵循 nix-evo skill)
    │
    │ bash (exec)
    │
...
早期设计（v0.1-v0.4）采用了双组件架构：nix-evo-agent (Rust HTTP) + nix-evo MCP Server (TypeScript)。代码保留在 `evo/` 和 `mcp-server/`。
